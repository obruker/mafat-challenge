...
from vehicle_crop.diagonal_crop import *  # pylint: disable=wildcard-import
from vehicle_crop.diagonal_crop.util import *  # pylint: disable=wildcard-import
...
def crop(im, base, angle, height, width):
    ...
    return cropWithPoints(im, angle, points)


def cropWithPoints(im, angle, points):
    ...
    im3 = im2.rotate(-angle * 180 / math.pi, resample=Image.BICUBIC, expand=True)
